...
**What the gate does not do.** It judges relevance, not need. The utility-aware experiments measured the consequence: on host-issued searches the dense-score distributions for "memory applies" and "ordinary turn" overlap almost completely, so no `dense_floor` separates them. This is a property of the signal, not of the chosen number. [usefulness-gate.md](usefulness-gate.md) records the durable result and the replacement. A record about the user's coffee habit passes on any coffee query, and whether that is personalization or pollution depends on the task. In `tool_only` mode the model absorbs that judgement by deciding to search. Host-issued conditional memory instead uses the planner and admission policy described in [utility-aware-memory-architecture.md](utility-aware-memory-architecture.md).
...
| Reranker floor | Leave it unset and reject `reranker.enabled` without one. The calibration found 0.01 as a recall-oriented candidate control and found that reranking cost explicit recall in the utility-aware path in both placements, after the RRF floors and in place of them, so the stage stays off; refer to [usefulness-gate.md](usefulness-gate.md). | A retrieval evaluation shows a candidate pool the judge cannot handle without a pair-score cut. |
