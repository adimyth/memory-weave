...
from memory_weave.models import CandidateRecord, EntityStatus, ExtractionContext, Principal, Record, Scope, Turn
from memory_weave.policy import private_scope, readable_scopes, writable_scopes
from memory_weave.policy.activation import ActivationService
from memory_weave.store import SessionRow, Store
from memory_weave.util import Timer, normalize_alias, now
...
class ExtractionRunner:
    ...
    def schedule(self, session_id: str, principal: Principal, *, force: bool = False) -> threading.Thread:
        ...
        thread = threading.Thread(
            ...
            kwargs={"force": force},
            name=f"memory-weave-extract-{session_id}",
            daemon=True,
        )
        thread.start()
        return thread
...
def _summary_text(content: str, decisions: list[str], config: MemoryWeaveConfig) -> str:
    text = content.strip()
