...
from memory_weave import runtime
from memory_weave.config import MemoryWeaveConfig, load_config
from memory_weave.host import MemoryHost
from memory_weave.models import Principal, Scope, ScopeKind, SearchRequest
...
def _dispatch(
    parser: argparse.ArgumentParser, store: Store, config: MemoryWeaveConfig, args: argparse.Namespace
) -> int:
...
def _search(store: Store, config: MemoryWeaveConfig, args: argparse.Namespace) -> int:
...
def _get(store: Store, config: MemoryWeaveConfig, args: argparse.Namespace) -> int:
    components = runtime.build_runtime(config, store)
...
def _reembed(store: Store, config: MemoryWeaveConfig, operations: Operations, args: argparse.Namespace) -> int:
...
def _extract(store: Store, config: MemoryWeaveConfig, args: argparse.Namespace) -> int:
...
def _snapshot_load(args: argparse.Namespace, config: MemoryWeaveConfig) -> int:
    if not args.yes:
