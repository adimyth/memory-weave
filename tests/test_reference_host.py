...
_BUNDLE = {"planner": "fake/gap-1", "judge": "fake/adm-1", "classifier": "fake/cat-1", "taxonomy": "t1", "inventory_builder": "i1", "retrieval_config_sha256": "abc"}
...
class Admission:
    ...
    def admit(self, turn, public_context, ambient_profile, draft, candidates):
        verdicts = [CandidateVerdict(r.id, "helpful" if self.should_admit else "insufficient", "fake") for r in candidates]
...
def world(tmp_path: Path):
    ...
    store.insert_record(
        Record(id="tz", type="semantic", version=1, content="Time zone is Europe/Berlin.", subject=render_subject(entity, "tz"),
               scope=Scope(kind="user", id="u"), source_kind="user_statement", source_ref=None, creator_agent_id="agent",
               evidence="x", created_at=_AT, event_at=_AT, expires_at=None, confidence=0.9, status="confirmed", supersedes_id=None,
               reinforcements=0, last_reinforced_at=None, tags=[], entity_ids=[entity], subject_entity_id=entity, attribute="tz", category="time_zone")
    )
    return store, principal


def _host(store, gaps, admission, **kwargs) -> ReferenceHost:
    return ReferenceHost(store, retrieve=lambda p, q, c: [store.get_record("tz")], gap_policy=gaps, admission_policy=admission, bundle=_BUNDLE, **kwargs)
...
def test_kill_switches_are_independent_and_change_the_bundle_hash(world) -> None:
    ...
    for switches in (KillSwitches(), KillSwitches(admission=False), KillSwitches(gap=False)):
        cfg = replace(probe.config(), gap_enabled=switches.gap, admission_mode="hosted_judge" if switches.admission else "disabled", shadow=not switches.regeneration)
        _approve(store, cfg)
...
def test_rollback_disables_the_newest_stage_on_breach_and_logs_it(world) -> None:
    ...
    calm = _host(store, Gaps(False), Admission(False), switches=KillSwitches(regeneration=False), thresholds=RollbackThresholds(min_turns=1000))
