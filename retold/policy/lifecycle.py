...
from retold.models import Record, RecordStatus, SourceKind
...
def rank(source: SourceKind | Record, config: RetoldConfig) -> int:
...
def provisional_expiry(current_time: datetime, config: RetoldConfig) -> datetime:
...
def reinforce(record: Record, current_time: datetime, config: RetoldConfig) -> Record:
...
def has_authority(new: Record, old: Record, config: RetoldConfig) -> bool:
